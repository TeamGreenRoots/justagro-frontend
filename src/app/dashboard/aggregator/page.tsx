...
import { Package, Users, TrendingUp, DollarSign, Clock, CheckCircle, Plus, Copy, Share2, ExternalLink, Loader2, Sparkles } from "lucide-react";
...
export default function AggregatorDashboard() {
  ...
  // Auto-load market summary on mount
  useEffect(() => { loadMarketSummary(); }, []);
    ...
    <DashboardLayout title="Platform Overview">
      ...
      <div className="bg-gradient-to-r from-emerald-900 to-brand-900 rounded-2xl p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          <p className="text-white font-semibold flex items-center gap-2"><Sparkles className="w-4 h-4 text-gold-400" /> AI Market Intelligence</p>
        ...
        <div className="divide-y divide-slate-50">
          {!(recentTransactions || []).some((t: Transaction) => ["PAID","ASSISTED"].includes(t.status)) ? (
            <div className="flex flex-col items-center justify-center py-12 text-center px-6">
              ...
              <p className="font-semibold text-slate-600 text-sm mb-1">No completed payments yet</p>
              <p className="text-slate-400 text-xs leading-relaxed max-w-xs">
                Completed transactions will appear here once buyers pay via Interswitch.
              </p>
            </div>
          ) : (
            (recentTransactions || [])
